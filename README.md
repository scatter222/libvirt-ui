...
The machine the app runs on can be in one of several states. That state is a
single line in **`/usr/share/mode`**, written by whatever provisions the
machine:
...
The main process reads it once at startup (`src/modes/appMode.ts`) and passes it
to the renderer through the preload bridge, so it is available on the first
render - no loading state. Anywhere in the UI:

```tsx
import { useAppMode } from '@/app/context/app-mode-provider';

const mode = useAppMode(); // e.g. 'training', or null if the file is missing
```

The tab bar shows the current mode as a badge.
